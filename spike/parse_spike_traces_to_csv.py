#!/usr/bin/env python3
"""
Parse LowBit Trace Corpus *.spike.txt files into an ML-ready CSV.
...
TRACES_DIR = Path(sys.argv[1]) if len(sys.argv) > 1 else SCRIPT_DIR / "spike_traces"
OUTPUT_CSV = Path(sys.argv[2]) if len(sys.argv) > 2 else SCRIPT_DIR / "lowbit_traces_spike.csv"
...
# Exact retired-instruction count derived from spike's own
# --log-commits golden-model trace (see run_spike.sh). This has zero
# sampling error, unlike perf-derived counts -- it's the corpus's most
# reliable instruction count wherever it's present.
EXACT_INSTRET_PATTERN = re.compile(r'spike_exact_instructions_retired=(\d+)')
...
def parse_perf_file(filepath):
    ...
    # The spike backend never invokes perf at all (see run_spike.sh --
    # spike has no perf integration; only self-reported program output
    # is collected here). The shared negative-match heuristic below
    # (inherited from the qemu/native parser template) can only detect
    # perf explicitly failing to launch, not perf never being invoked
    # in the first place -- so it defaults to 1 for every spike trace,
    # which is wrong. Force it to 0 unconditionally for this backend.
    ...
    # Everything below this point is a counter spike genuinely cannot
    # produce (no timing model -- see run_spike.sh header). Rather
    # than writing a real-looking 0 (which reads as "measured zero
    # events", not "not applicable"), these are left as None so the
    # CSV writer emits an empty cell / pandas reads them as NaN.
    ...
    # instructions_total is the ONE counter spike can give us reliably
    # and exactly, via its own golden-model commit log (see
    # run_spike.sh SPIKE_LOG_COMMITS). Use it when present; otherwise
    # leave it None rather than the old always-0 placeholder.
    instret_match = EXACT_INSTRET_PATTERN.search(content)
    data['instructions_total'] = int(instret_match.group(1)) if instret_match else None
    data['instruction_count_source'] = 'spike_exact_commit_log' if instret_match else ''

    # "plausible" here means "did we get an exact instruction count at
    # all", not the branches_to_N sanity check the other backends use
    # (spike doesn't report branches_total, so that check doesn't
    # apply). validation=PASS (functional correctness) is tracked
    # separately and is unaffected by any of this.
    data['plausible'] = 1 if instret_match else 0
...
def aggregate(rows):
    ...
    # instructions_total is the only counter-derived field spike can
    # produce (via --log-commits); average it across reps that
    # actually got an exact count. Everything else stays None -- spike
    # has no timing model, so there's nothing to average.
    numeric_avg_fields = [
        'instructions_total', 'task_clock_ms', 'context_switches',
        'cpu_migrations', 'page_faults', 'time_elapsed',
    ]
    for f in numeric_avg_fields:
        vals = [r[f] for r in rows if r.get(f) is not None]
        base[f] = round(statistics.mean(vals), 6) if vals else None

    always_none_fields = [
        'cycles_total', 'branches_total', 'branch_misses_total',
        'cache_references_total', 'cache_misses_total',
        'observed_ipc', 'observed_cpi', 'observed_branch_miss_rate',
        'observed_cache_miss_rate', 'observed_branch_density',
        'observed_cache_refs_per_instruction', 'branches_to_N_ratio',
    ]
    for f in always_none_fields:
        base[f] = None

    base['perf_available'] = 0
    base['plausible'] = 1 if all(r['plausible'] for r in rows) else 0
...
def main():
    ...
    trace_files = sorted(TRACES_DIR.glob("*.spike.txt"))
