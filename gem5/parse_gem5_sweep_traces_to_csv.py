...
Handles both directory layouts with the same code: run_gem5_MC.sh's
flat <STATS_DIR>/<program>/ and run_gem5_MC_sweep.sh's nested
<STATS_DIR>/seed*/size*/bp*/<program>/ -- any directory containing
meta.txt is a leaf run, found via a recursive walk, at any depth.
Four new columns carry the sweep identity: `bp_type` (cleaned, e.g.
'LTAGE'), `bp_type_raw` (the full descriptive string, unchanged),
`size_override`, and `seed_override` (blank for pre-sweep runs that
never recorded a seed). One combined CSV covers everything found
under STATS_DIR -- group by seed_override/size_override/bp_type
afterward (pandas groupby or a CSV pivot) rather than parsing each
combination into a separate file; a single file is what a size x
predictor interaction check needs anyway.
...
Usage:
    ...
    python3 parse_gem5_traces_to_csv.py gem5_stats_minorcpu   # sweep dir also OK, output defaults next to it
"""
...
STATS_DIR = Path(ARGS[0]) if len(ARGS) > 0 else SCRIPT_DIR / "gem5_stats_minorcpu_sweep"
# Default output sits next to STATS_DIR (its parent), same convention
# the corpus already uses (lowbit_traces_gem5_minorcpu.csv next to
# gem5_stats_minorcpu/) -- not next to this script, which breaks if
# you point it at a stats dir elsewhere.
OUTPUT_CSV = Path(ARGS[1]) if len(ARGS) > 1 else STATS_DIR.parent / f"lowbit_traces_{STATS_DIR.name.replace('gem5_stats_', 'gem5_')}.csv"
...
CORE_FIELDNAMES = [
    'run_dir', 'program', 'backend', 'cpu_type', 'bp_type', 'bp_type_raw',
    'l1d_size', 'l1i_size', 'l2_size', 'size_override', 'n_override', 'seed_override',
    'stats_available',
...
META_PATTERNS = {
    ...
    r'^size_override=(\d+)$': 'size_override',
    r'^n_override=(\d+)$': 'n_override',
    r'^seed_override=(\d+)$': 'seed_override',
}
L1_L2_SIZE_PATTERN = re.compile(r'l1d_size=(\S+)\s+l1i_size=(\S+)\s+l2_size=(\S+)')
INT_META_FIELDS = {'n_override', 'size_override', 'seed_override'}


def clean_bp_type(bp_type_raw):
    """bp_type_raw is a full descriptive string, e.g. 'LTAGE (applied:
    yes)' or 'default (CPU model's built-in predictor; GEM5_BP_TYPE
    not set)' -- bp_type is just the short name a groupby/pivot wants:
    'LTAGE', 'default', or '<name> (unapplied)' when run_gem5_MC_sweep.sh
    logged that this build's se.py didn't support the requested one."""
    if not bp_type_raw:
        return ''
    first = bp_type_raw.split()[0]
    if first == 'default':
        return 'default'
    if 'applied: no' in bp_type_raw:
        return f'{first} (unapplied)'
    return first


def parse_meta_txt(filepath):
    meta = {'cpu_type': '', 'bp_type_raw': '', 'n_override': 0, 'size_override': '',
            'seed_override': '', 'l1d_size': '', 'l1i_size': '', 'l2_size': ''}
    ...
    for regex, key in META_PATTERNS.items():
        ...
        if match:
            ...
            meta[key] = int(val) if key in INT_META_FIELDS else val
    return meta
...
def parse_one(run_dir, prog_dir):
    ...
    row['bp_type'] = clean_bp_type(meta['bp_type_raw'])
    row['l1d_size'] = meta['l1d_size']
    row['l1i_size'] = meta['l1i_size']
    row['l2_size'] = meta['l2_size']
    row['size_override'] = meta['size_override']
    row['n_override'] = meta['n_override']
    row['seed_override'] = meta['seed_override']
...
def find_run_dirs(stats_dir):
    """Any directory containing meta.txt is a leaf run-dir, at any
    depth -- this is what makes both run_gem5_MC.sh's flat layout
    (<STATS_DIR>/<program>/) and run_gem5_MC_sweep.sh's nested one
    (<STATS_DIR>/seed*/size*/bp*/<program>/) parse with the same code,
    with no separate flat-vs-nested branch needed."""
    return sorted({p.parent for p in stats_dir.rglob('meta.txt')})
...
def main():
    ...
    run_dirs = find_run_dirs(STATS_DIR)
    print(f"Found {len(run_dirs)} run folders under {STATS_DIR} (any depth).")
    if FULL:
        print("--full: also writing raw_stats_json (every numeric stat gem5 dumped)")

    n_no_stats = 0
    n_unmatched = 0
    seeds, sizes, bps = set(), set(), set()
    with open(OUTPUT_CSV, 'a+', newline='', encoding='utf-8') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=FIELDNAMES)
        writer.writeheader()
        for d in run_dirs:
            run_dir_key = d.relative_to(STATS_DIR).as_posix()
            row = parse_one(run_dir_key, d)
            row['program'] = d.name  # leaf folder name only; run_dir keeps the full path unique
            if not row['stats_available']:
                n_no_stats += 1
            elif row['unmatched_metrics']:
                n_unmatched += 1
            if row['seed_override'] != '':
                seeds.add(row['seed_override'])
            if row['size_override'] != '':
                sizes.add(row['size_override'])
            if row['bp_type']:
                bps.add(row['bp_type'])
            writer.writerow(row)
    ...
    if len(seeds) > 1 or len(sizes) > 1 or len(bps) > 1:
        print(f"   Sweep detected: {len(seeds) or 1} seed(s), {len(sizes) or 1} size(s), "
              f"{len(bps) or 1} predictor(s) -- group by seed_override/size_override/bp_type")
        print(f"     for the size x predictor interaction check before trusting a")
        print(f"     pooled average across all of them.")
