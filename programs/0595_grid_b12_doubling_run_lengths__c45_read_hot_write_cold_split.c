/*
 * LowBit -- 0595_grid_b12_doubling_run_lengths__c45_read_hot_write_cold_split.c
 * Branch: doubling_run_lengths | Cache: read_hot_write_cold_split
...
int main(void) {
    ...
    int *prev_taken = &prev_taken_local;
        ...
        uint32_t half45_ = (SIZE / 2u) > 0u ? SIZE / 2u : 1u;
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = (((i % 5u) != 0u) ? (uint32_t)(i % half45_) : (uint32_t)(half45_ + (i % half45_)));
        uint32_t v = buf[idx];
        int taken;
  ...
  if (run_pos_ >= run_len_) { run_pos_ = 0; run_len_ = (run_len_ < 4096u) ? run_len_ * 2u : 1u; dir_ ^= 1u; } }
    ...
    printf("program=0595_grid_b12_doubling_run_lengths__c45_read_hot_write_cold_split.c\n");
    printf("branch_pattern=doubling_run_lengths\n");
    printf("cache_pattern=read_hot_write_cold_split\n");
