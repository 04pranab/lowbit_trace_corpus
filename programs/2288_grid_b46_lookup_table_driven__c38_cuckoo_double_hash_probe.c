/*
 * LowBit -- 2288_grid_b46_lookup_table_driven__c38_cuckoo_double_hash_probe.c
 * Branch: lookup_table_driven | Cache: cuckoo_double_hash_probe
...
int main(void) {
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = ((((i) & 1u) == 0u) ? (uint32_t)((i * 2654435761u) % SIZE) : (uint32_t)((i * 40503u + 1u) % SIZE));
        uint32_t v = buf[idx];
        int taken;
  ...
  taken = tbl46_[v & 0xFFu]; }
    ...
    printf("program=2288_grid_b46_lookup_table_driven__c38_cuckoo_double_hash_probe.c\n");
    printf("branch_pattern=lookup_table_driven\n");
    printf("cache_pattern=cuckoo_double_hash_probe\n");
    printf("predictor_stress_type=stochastic\n");
    printf("expected_dominant_miss_type=capacity+compulsory (indirect gather/scatter)\n");
