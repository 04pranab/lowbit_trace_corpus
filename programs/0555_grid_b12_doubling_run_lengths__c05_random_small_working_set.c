/*
 * LowBit -- 0555_grid_b12_doubling_run_lengths__c05_random_small_working_set.c
 * Branch: doubling_run_lengths | Cache: random_small_working_set
...
int main(void) {
    ...
    int *prev_taken = &prev_taken_local;
        ...
        uint32_t cseed_ = SEED0 ^ 0xABCDEFu; uint32_t small_mask_ = (SIZE > 4096u ? 4096u : SIZE) - 1u;
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = (uint32_t)(lcg_next(&cseed_) & small_mask_);
        uint32_t v = buf[idx];
        int taken;
  ...
  if (run_pos_ >= run_len_) { run_pos_ = 0; run_len_ = (run_len_ < 4096u) ? run_len_ * 2u : 1u; dir_ ^= 1u; } }
    ...
    printf("program=0555_grid_b12_doubling_run_lengths__c05_random_small_working_set.c\n");
    printf("branch_pattern=doubling_run_lengths\n");
    printf("cache_pattern=random_small_working_set\n");
    printf("predictor_stress_type=phase-changing/non-stationary\n");
    printf("expected_dominant_miss_type=temporal reuse (capacity-sensitive)\n");
