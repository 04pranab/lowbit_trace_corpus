/*
 * LowBit -- 0319_grid_b07_nested_dependent__c19_matrix_diagonal_wavefront.c
 * Branch: nested_dependent | Cache: matrix_diagonal_wavefront
...
int main(void) {
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
     ...
     (r_ < mrows19_ && c_ < mcols19_) ? (r_ * mcols19_ + c_) : (uint32_t)(i % SIZE); });
        uint32_t v = buf[idx];
        int taken;
        { uint32_t outer_ = (uint32_t)((i >> 4) & 1u);
  ...
  taken = inner_ < (outer_ ? 12u : 4u); }
    ...
    printf("program=0319_grid_b07_nested_dependent__c19_matrix_diagonal_wavefront.c\n");
    printf("branch_pattern=nested_dependent\n");
