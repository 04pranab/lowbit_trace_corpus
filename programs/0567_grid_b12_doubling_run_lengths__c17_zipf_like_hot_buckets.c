/*
 * LowBit -- 0567_grid_b12_doubling_run_lengths__c17_zipf_like_hot_buckets.c
 * Branch: doubling_run_lengths | Cache: zipf_like_hot_buckets
...
int main(void) {
    ...
    int *prev_taken = &prev_taken_local;
        ...
        uint32_t cseed17_ = SEED0 ^ 0x777333u; uint32_t hot_size_ = (SIZE > 256u ? 256u : SIZE);
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = ({ uint32_t r_ = lcg_next(&cseed17_) % 10u;
  ...
  if (run_pos_ >= run_len_) { run_pos_ = 0; run_len_ = (run_len_ < 4096u) ? run_len_ * 2u : 1u; dir_ ^= 1u; } }
    ...
    printf("program=0567_grid_b12_doubling_run_lengths__c17_zipf_like_hot_buckets.c\n");
    printf("branch_pattern=doubling_run_lengths\n");
    printf("cache_pattern=zipf_like_hot_buckets\n");
