/*
 * LowBit -- 2264_grid_b46_lookup_table_driven__c14_temporal_burst_reuse.c
 * Branch: lookup_table_driven | Cache: temporal_burst_reuse
...
int main(void) {
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = (uint32_t)(((i / BURST_) * 131u) % SIZE);
        uint32_t v = buf[idx];
        int taken;
  ...
  taken = tbl46_[v & 0xFFu]; }
    ...
    printf("program=2264_grid_b46_lookup_table_driven__c14_temporal_burst_reuse.c\n");
    printf("branch_pattern=lookup_table_driven\n");
    printf("cache_pattern=temporal_burst_reuse\n");
