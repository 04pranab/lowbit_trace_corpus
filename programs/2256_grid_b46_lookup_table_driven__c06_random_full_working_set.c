/*
 * LowBit -- 2256_grid_b46_lookup_table_driven__c06_random_full_working_set.c
 * Branch: lookup_table_driven | Cache: random_full_working_set
...
int main(void) {
    ...
    int *prev_taken = &prev_taken_local;
        ...
        uint32_t cseed_ = SEED0 ^ 0x13572468u;
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = (uint32_t)(lcg_next(&cseed_) % SIZE);
        uint32_t v = buf[idx];
        int taken;
  ...
  taken = tbl46_[v & 0xFFu]; }
    ...
    printf("program=2256_grid_b46_lookup_table_driven__c06_random_full_working_set.c\n");
    printf("branch_pattern=lookup_table_driven\n");
    printf("cache_pattern=random_full_working_set\n");
