/*
 * LowBit -- 0569_grid_b12_doubling_run_lengths__c19_matrix_diagonal_wavefront.c
 * Branch: doubling_run_lengths | Cache: matrix_diagonal_wavefront
...
int main(void) {
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
     ...
     (r_ < mrows19_ && c_ < mcols19_) ? (r_ * mcols19_ + c_) : (uint32_t)(i % SIZE); });
        uint32_t v = buf[idx];
        int taken;
        { static uint64_t run_pos_ = 0, run_len_ = 1, dir_ = 1;
  taken = (int)dir_;
  run_pos_++;
  if (run_pos_ >= run_len_) { run_pos_ = 0; run_len_ = (run_len_ < 4096u) ? run_len_ * 2u : 1u; dir_ ^= 1u; } }
  ...
  if (run_pos_ >= run_len_) { run_pos_ = 0; run_len_ = (run_len_ < 4096u) ? run_len_ * 2u : 1u; dir_ ^= 1u; } }
    ...
    printf("program=0569_grid_b12_doubling_run_lengths__c19_matrix_diagonal_wavefront.c\n");
    printf("branch_pattern=doubling_run_lengths\n");
    printf("cache_pattern=matrix_diagonal_wavefront\n");
    printf("predictor_stress_type=phase-changing/non-stationary\n");
