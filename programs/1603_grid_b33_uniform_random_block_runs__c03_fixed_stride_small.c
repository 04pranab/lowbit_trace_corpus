/*
 * LowBit -- 1603_grid_b33_uniform_random_block_runs__c03_fixed_stride_small.c
 * Branch: uniform_random_block_runs | Cache: fixed_stride_small
...
int main(void) {
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = (uint32_t)((i * 8u) % SIZE);
        uint32_t v = buf[idx];
        int taken;
  ...
  if (pos33_ >= len33_) { pos33_ = 0; len33_ = 1u + (lcg_next(&bseed33_) % 64u); dir33_ ^= 1; } }
    ...
    printf("program=1603_grid_b33_uniform_random_block_runs__c03_fixed_stride_small.c\n");
    printf("branch_pattern=uniform_random_block_runs\n");
    printf("cache_pattern=fixed_stride_small\n");
