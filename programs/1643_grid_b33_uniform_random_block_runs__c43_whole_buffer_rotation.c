/*
 * LowBit -- 1643_grid_b33_uniform_random_block_runs__c43_whole_buffer_rotation.c
 * Branch: uniform_random_block_runs | Cache: whole_buffer_rotation
...
int main(void) {
    ...
    int *prev_taken = &prev_taken_local;
        ...
        const uint64_t ROT_PERIOD43_ = SIZE; uint32_t rot_offset43_ = 0u;
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = ({ rot_offset43_ = (uint32_t)((i / ROT_PERIOD43_) * 97u) % SIZE;
  ...
  if (pos33_ >= len33_) { pos33_ = 0; len33_ = 1u + (lcg_next(&bseed33_) % 64u); dir33_ ^= 1; } }
    ...
    printf("program=1643_grid_b33_uniform_random_block_runs__c43_whole_buffer_rotation.c\n");
    printf("branch_pattern=uniform_random_block_runs\n");
    printf("cache_pattern=whole_buffer_rotation\n");
