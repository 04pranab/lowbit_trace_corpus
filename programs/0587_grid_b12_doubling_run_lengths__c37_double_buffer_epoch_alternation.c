/*
 * LowBit -- 0587_grid_b12_doubling_run_lengths__c37_double_buffer_epoch_alternation.c
 * Branch: doubling_run_lengths | Cache: double_buffer_epoch_alternation
...
int main(void) {
    ...
    int *prev_taken = &prev_taken_local;
        ...
        uint32_t half37_ = (SIZE / 2u) > 0u ? SIZE / 2u : 1u;
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = ((((i / half37_) & 1u) == 0u) ? (uint32_t)(i % half37_) : (uint32_t)(half37_ + (i % half37_)));
        uint32_t v = buf[idx];
        int taken;
  ...
  if (run_pos_ >= run_len_) { run_pos_ = 0; run_len_ = (run_len_ < 4096u) ? run_len_ * 2u : 1u; dir_ ^= 1u; } }
    ...
    printf("program=0587_grid_b12_doubling_run_lengths__c37_double_buffer_epoch_alternation.c\n");
    printf("branch_pattern=doubling_run_lengths\n");
    printf("cache_pattern=double_buffer_epoch_alternation\n");
    printf("predictor_stress_type=phase-changing/non-stationary\n");
    printf("expected_dominant_miss_type=conflict+capacity (thrashing/alternation)\n");
