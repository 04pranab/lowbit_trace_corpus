/*
 * LowBit -- 1620_grid_b33_uniform_random_block_runs__c20_hot_cold_phase_alternation.c
 * Branch: uniform_random_block_runs | Cache: hot_cold_phase_alternation
...
int main(void) {
    ...
    int *prev_taken = &prev_taken_local;
        ...
        const uint64_t PHASE_LEN_ = N / 8u > 0u ? N / 8u : 1u;
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = ((((i / PHASE_LEN_) & 1u) == 0u) ? (uint32_t)(i % hot_size20_) : (uint32_t)(lcg_next(&cseed20_) % SIZE));
        uint32_t v = buf[idx];
        int taken;
  ...
  if (pos33_ >= len33_) { pos33_ = 0; len33_ = 1u + (lcg_next(&bseed33_) % 64u); dir33_ ^= 1; } }
    ...
    printf("program=1620_grid_b33_uniform_random_block_runs__c20_hot_cold_phase_alternation.c\n");
    printf("branch_pattern=uniform_random_block_runs\n");
    printf("cache_pattern=hot_cold_phase_alternation\n");
    printf("predictor_stress_type=stochastic\n");
    printf("expected_dominant_miss_type=temporal reuse (skewed/hot-set)\n");
