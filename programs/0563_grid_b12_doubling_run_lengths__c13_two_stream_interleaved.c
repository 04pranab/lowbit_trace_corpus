/*
 * LowBit -- 0563_grid_b12_doubling_run_lengths__c13_two_stream_interleaved.c
 * Branch: doubling_run_lengths | Cache: two_stream_interleaved
...
int main(void) {
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = ((i & 1u) == 0u) ? (uint32_t)((i / 2u) % half_) : (uint32_t)(half_ + ((i / 2u) % half_));
        uint32_t v = buf[idx];
        int taken;
  ...
  if (run_pos_ >= run_len_) { run_pos_ = 0; run_len_ = (run_len_ < 4096u) ? run_len_ * 2u : 1u; dir_ ^= 1u; } }
    ...
    printf("program=0563_grid_b12_doubling_run_lengths__c13_two_stream_interleaved.c\n");
    printf("branch_pattern=doubling_run_lengths\n");
    printf("cache_pattern=two_stream_interleaved\n");
