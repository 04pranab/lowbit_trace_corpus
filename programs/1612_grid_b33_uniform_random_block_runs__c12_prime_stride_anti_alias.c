/*
 * LowBit -- 1612_grid_b33_uniform_random_block_runs__c12_prime_stride_anti_alias.c
 * Branch: uniform_random_block_runs | Cache: prime_stride_anti_alias
...
int main(void) {
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = (uint32_t)((i * 97u) % SIZE);
        uint32_t v = buf[idx];
        int taken;
  ...
  if (pos33_ >= len33_) { pos33_ = 0; len33_ = 1u + (lcg_next(&bseed33_) % 64u); dir33_ ^= 1; } }
    ...
    printf("program=1612_grid_b33_uniform_random_block_runs__c12_prime_stride_anti_alias.c\n");
    printf("branch_pattern=uniform_random_block_runs\n");
    printf("cache_pattern=prime_stride_anti_alias\n");
