/*
 * LowBit -- 0559_grid_b12_doubling_run_lengths__c09_matrix_tiled_blocked.c
 * Branch: doubling_run_lengths | Cache: matrix_tiled_blocked
...
int main(void) {
    ...
    int *prev_taken = &prev_taken_local;
        ...
        uint32_t mrows_ = 1u; uint32_t mcols_ = SIZE;
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = ({ uint32_t tile_lin_ = (uint32_t)((i / elems_per_tile_) % (tiles_per_row_ * tiles_per_col_));
  ...
  if (run_pos_ >= run_len_) { run_pos_ = 0; run_len_ = (run_len_ < 4096u) ? run_len_ * 2u : 1u; dir_ ^= 1u; } }
    ...
    printf("program=0559_grid_b12_doubling_run_lengths__c09_matrix_tiled_blocked.c\n");
    printf("branch_pattern=doubling_run_lengths\n");
    printf("cache_pattern=matrix_tiled_blocked\n");
