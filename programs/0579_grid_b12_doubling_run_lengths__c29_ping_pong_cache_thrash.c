/*
 * LowBit -- 0579_grid_b12_doubling_run_lengths__c29_ping_pong_cache_thrash.c
 * Branch: doubling_run_lengths | Cache: ping_pong_cache_thrash
...
int main(void) {
    ...
    int *prev_taken = &prev_taken_local;
        ...
        const uint64_t PPHASE29_ = 5000u;
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = ((((i / PPHASE29_) & 1u) == 0u) ? (uint32_t)(regA29_ + (i % half29_)) : (uint32_t)(regB29_ + (i % half29_)));
        uint32_t v = buf[idx];
        int taken;
  ...
  if (run_pos_ >= run_len_) { run_pos_ = 0; run_len_ = (run_len_ < 4096u) ? run_len_ * 2u : 1u; dir_ ^= 1u; } }
    ...
    printf("program=0579_grid_b12_doubling_run_lengths__c29_ping_pong_cache_thrash.c\n");
    printf("branch_pattern=doubling_run_lengths\n");
    printf("cache_pattern=ping_pong_cache_thrash\n");
