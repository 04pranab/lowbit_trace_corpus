/*
 * LowBit -- 1635_grid_b33_uniform_random_block_runs__c35_checkerboard_red_black.c
 * Branch: uniform_random_block_runs | Cache: checkerboard_red_black
...
int main(void) {
    ...
    int *prev_taken = &prev_taken_local;
        ...
        uint32_t mrows35_ = 1u; uint32_t mcols35_ = SIZE;
        for (uint32_t d_ = 2u; d_ * d_ <= SIZE; d_++) { if (SIZE % d_ == 0u) mrows35_ = d_; }
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = ({ uint32_t lin35_ = (uint32_t)(i % ncells35_);
  ...
  if (pos33_ >= len33_) { pos33_ = 0; len33_ = 1u + (lcg_next(&bseed33_) % 64u); dir33_ ^= 1; } }
    ...
    printf("program=1635_grid_b33_uniform_random_block_runs__c35_checkerboard_red_black.c\n");
    printf("branch_pattern=uniform_random_block_runs\n");
    printf("cache_pattern=checkerboard_red_black\n");
