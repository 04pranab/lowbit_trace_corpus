/*
 * LowBit -- 1619_grid_b33_uniform_random_block_runs__c19_matrix_diagonal_wavefront.c
 * Branch: uniform_random_block_runs | Cache: matrix_diagonal_wavefront
...
int main(void) {
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
     ...
     (r_ < mrows19_ && c_ < mcols19_) ? (r_ * mcols19_ + c_) : (uint32_t)(i % SIZE); });
        uint32_t v = buf[idx];
        int taken;
        { static uint64_t pos33_ = 0, len33_ = 1; static int dir33_ = 1;
  static uint32_t bseed33_ = 0x33333333u;
  taken = dir33_;
  pos33_++;
  if (pos33_ >= len33_) { pos33_ = 0; len33_ = 1u + (lcg_next(&bseed33_) % 64u); dir33_ ^= 1; } }
  ...
  if (pos33_ >= len33_) { pos33_ = 0; len33_ = 1u + (lcg_next(&bseed33_) % 64u); dir33_ ^= 1; } }
    ...
    printf("program=1619_grid_b33_uniform_random_block_runs__c19_matrix_diagonal_wavefront.c\n");
    printf("branch_pattern=uniform_random_block_runs\n");
