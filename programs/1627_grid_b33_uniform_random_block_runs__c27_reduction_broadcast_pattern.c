/*
 * LowBit -- 1627_grid_b33_uniform_random_block_runs__c27_reduction_broadcast_pattern.c
 * Branch: uniform_random_block_runs | Cache: reduction_broadcast_pattern
...
int main(void) {
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = (((i % 4u) == 0u) ? (uint32_t)(i % NACC27_) : (uint32_t)(i % SIZE));
        uint32_t v = buf[idx];
        int taken;
  ...
  if (pos33_ >= len33_) { pos33_ = 0; len33_ = 1u + (lcg_next(&bseed33_) % 64u); dir33_ ^= 1; } }
    ...
    printf("program=1627_grid_b33_uniform_random_block_runs__c27_reduction_broadcast_pattern.c\n");
    printf("branch_pattern=uniform_random_block_runs\n");
    printf("cache_pattern=reduction_broadcast_pattern\n");
