/*
 * LowBit -- 1623_grid_b33_uniform_random_block_runs__c23_struct_of_array_field_burst.c
 * Branch: uniform_random_block_runs | Cache: struct_of_array_field_burst
...
int main(void) {
    ...
    int *prev_taken = &prev_taken_local;
        ...
        const uint32_t STRUCT_STRIDE23_ = 20u; const uint32_t FIELDS23_ = 3u;
    ...
    for (uint64_t i = 0; i < warmup_n; i++) {
        uint32_t idx = (uint32_t)((((i / FIELDS23_) * STRUCT_STRIDE23_) + (i % FIELDS23_)) % SIZE);
        uint32_t v = buf[idx];
        int taken;
  ...
  if (pos33_ >= len33_) { pos33_ = 0; len33_ = 1u + (lcg_next(&bseed33_) % 64u); dir33_ ^= 1; } }
    ...
    printf("program=1623_grid_b33_uniform_random_block_runs__c23_struct_of_array_field_burst.c\n");
    printf("branch_pattern=uniform_random_block_runs\n");
    printf("cache_pattern=struct_of_array_field_burst\n");
